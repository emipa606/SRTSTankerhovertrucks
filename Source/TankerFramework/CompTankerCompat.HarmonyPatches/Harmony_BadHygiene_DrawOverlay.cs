...
internal static class Harmony_BadHygiene_DrawOverlay
{
    ...
    {
        var type = AccessTools.TypeByName("Rimefeller.MapComponent_Hygiene");
        if (type == null)
        {
            return null;
        }
        ...
        pipedThingsField =
            AccessTools.FieldRefAccess<HashSet<ThingWithComps>>(AccessTools.TypeByName("Rimefeller.PlumbingNet"),
                "PipedThings");
    ...
    public static void Prefix(MapComponent __instance)
    {
        ...
        {
            foreach (var item2 in from x in pipedThingsField(item)
                     orderby x.thingIDNumber
                     select x)
            {
